...
class Helpers {
    ...
    public static function pivotTableDD($node, $data) {
        ...
        //contextual
        if(F::$engineNamespace == "admin/categories/index") {
            F::$db->sqlCommand = "SELECT DISTINCT fk_pivot_table FROM category";
            $dropDown->addOption("--- any ---", "");
            $dropDown->addOption("--- none ---", "0");

        }
        else if(F::$engineNamespace == "admin/categories/add-edit") {
