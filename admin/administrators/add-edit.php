	...
	class P {
		//--> Begin Method :: Event_BeforeActions
			public static function Event_BeforeActions() {
				//user id magic
				F::$SQLKeyBinders["fk_user_id"] = F::$Admin->GetUserID(F::$Request->Input("id"));
		...
		//--> Begin Method :: Action_CreateNew
			public static function Action_CreateNew() {
				...
				if(F::$Errors->Count() == 0) {
					...
					F::$DB->LoadCommand("create-new-admin", F::$PageInput);
					F::$DB->ExecuteNonQuery();
					$tmpAdminID = F::$DB->GetLastInsertID();
					
					F::$Response->RedirectURL = F::URL(F::$PageNamespace .".html?id=". $tmpAdminID);
				}
			}
		...
		//--> Begin Method :: Action_Update
			public static function Action_Update() {
				...
				if(F::$Errors->Count() == 0) {
					...
					F::$DB->LoadCommand("update-admin", F::$PageInput);
					F::$DB->ExecuteNonQuery();
		...
		//--> Begin Method :: Action_Delete
			public static function Action_Delete() {
				...
				if(F::$Errors->Count() == 0) {
					...
					F::$DB->LoadCommand("delete-admin", F::$PageInput);
					F::$DB->ExecuteNonQuery();
